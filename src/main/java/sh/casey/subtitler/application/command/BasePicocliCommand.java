...
abstract class BasePicocliCommand implements Runnable {
    ...
    @Option(names = {"--input-type"}, description = "The type of subtitle file to read from. If not specified, the file extension will be used. Valid options: ${COMPLETION-CANDIDATES}", completionCandidates = SubtitleTypeCompleter.class, paramLabel = "<type>")
    private SubtitleType inputType;

    @Option(names = {"--output-type"}, description = "The type of subtitle file to write to. If not specified, the input type will be used. Valid options: ${COMPLETION-CANDIDATES}", completionCandidates = SubtitleTypeCompleter.class, paramLabel = "<type>")
